...
<template>
  <div>
    <section>
      ...
      <p>
        Nuxt CLIには、開発モードでのdotenvのサポートと、nuxi buildおよびnuxi
        generateを実行する際に組み込まれたdotenvのサポートがあります。
      </p>
      <p>
        .nuxtディレクトリは開発時にVueアプリケーションを生成するために使用され、nuxi
        devを実行すると自動的に生成されます。このディレクトリ内のファイルを直接変更する必要はありません。
      </p>
      <div
      ...
      >
        ...
        <div>
          ...
          <p>
            上記のように、これは開発モードでのみ適用され、nuxi buildとnuxi generateを実行するときにのみ適用されます。
          </p>
        </div>
      </div>
      <p>
    ...
    <section>
      ...
      <p>
        サーバーを構築した後、サーバーを実行するときに環境変数を設定する必要があります。この時点では .env
        ファイルは読み取られません。これを行う方法は環境ごとに異なります。
      </p>
      <p>
        ローカル実稼働プレビューの目的には、nuxi
        プレビューを使用することをお勧めします。このコマンドを使用すると、利便性のために .env ファイルが process.env
        にロードされるためです。 このコマンドでは、依存関係をパッケージ
        ディレクトリにインストールする必要があることに注意してください。
      </p>
      <p>または、ターミナルを使用して環境変数を引数として渡すこともできます。 たとえば、Linux または macOS の場合:</p>
      ...
      <p>
        ビルド時に設定された環境変数を使用したいが、後で更新する必要がない
        (またはアプリ内で事後的に更新する必要があるだけ) 場合は、appConfig の方が良い選択肢になる可能性があります。
        appConfig は、nuxt.config 内 (環境変数を使用) と、プロジェクト内の ~/app.config.ts
        ファイル内の両方で定義できます。
      </p>
      <div
        class="flex p-4 mb-4 text-sm text-blue-800 rounded-lg bg-blue-50 dark:bg-gray-800 dark:text-blue-200"
