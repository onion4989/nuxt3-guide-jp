  ...
  <div>
    ...
    <section>
      ...
      >
        <span class="sr-only">Info</span>
        <div>
          オブジェクト構文プラグインを使用している場合、将来的にプロパティが静的に解析されて、より最適化されたビルドが生成される可能性があります。そのため、ランタイムで定義するべきではありません。例えば、enforce:
          process.server ? ‘pre’ :
          'post’と設定すると、Nuxtがプラグインに対して行うことができる将来の最適化を妨げることになります
        </div>
      </div>
    </section>
    ...
    <section>
      ...
      <p>ファイル名の先頭に「アルファベット」の番号を付けることで、登録の順序を制御することができます。</p>
      <p>For example:</p>
    ...
    <section>
      ...
      <p>
        デフォルトでは、Nuxt はプラグインを順番にロードします。 プラグインを並列として定義できるため、Nuxt
        はプラグインの実行の終了を待たずに次のプラグインをロードできます。
      </p>
    ...
    <section>
      ...
      <p>
        理由: プラグインは、他のすべての前に順番に呼び出されます。
        まだ呼び出されていない別のプラグインに依存するコンポーザブルを使用する可能性があります。
      </p>
      <p>コンポーザブルが Vue.js ライフサイクルに依存している場合、それは機能しません。</p>
      <p>
        理由: 通常、Vue.js コンポーザブルは現在のコンポーネント インスタンスにバインドされますが、プラグインは nuxtApp
        インスタンスにのみバインドされます。
      </p>
    </section>
    ...
    <section>
      ...
      <p>
        NuxtApp インスタンスにヘルパーを提供したい場合は、provide
        キーを使用してプラグインからヘルパーを返します。例えば：
      </p>
    ...
    <section>
      ...
      <p>
        プラグインからヘルパーを返す場合、ヘルパーは自動的に入力されます。 これらは useNuxtApp()
        の戻り値とテンプレート内に入力されていることがわかります。
      </p>
      <div
        class="flex p-4 mb-4 text-sm text-blue-800 rounded-lg bg-blue-50 dark:bg-gray-800 dark:text-blue-200"
        role="alert"
      >
        <span class="sr-only">Info</span>
        <div>
          別のプラグイン内で提供されたヘルパーを使用する必要がある場合は、useNuxtApp()
          を呼び出して型指定されたバージョンを取得できます。
          ただし、一般に、プラグインの順序が確実でない限り、これは避けるべきです。
        </div>
      </div>
    </section>
    ...
    <section>
      ...
      <p>
        Google
        Analyticsタグを追加するvue-gtagなどのVueプラグインを使用したい場合は、Nuxtプラグインを使用してこれを行うことができます。
      </p>
    ...
    <section>
      ...
      <p>
        同様に、カスタム Vue ディレクティブをプラグインに登録できます。 たとえば、plugins/directive.ts
        では次のようになります。
      </p>
